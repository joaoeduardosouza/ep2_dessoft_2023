...
while jogando:
    ...
    #se afundou todos os navios (funcao afundados) jogando = False e imprime mensagem
        if afundados(frota_oponente, tabuleiro_op) == 10:
            ...
            while [linha_op, coluna_op] in jogadas_op:
                linha_op = random.randint(0,9)
                coluna_op = random.randint(0,9)
             
            tabuleiro_jog = faz_jogada(tabuleiro_jog, linha_op, coluna_op)
            jogadas_op.append([linha_op, coluna_op])
            if afundados(frota, tabuleiro_jog) == 10:
